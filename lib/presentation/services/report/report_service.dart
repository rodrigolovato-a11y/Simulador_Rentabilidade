...
class ReportService {
  static Future<Uint8List> buildSimulationPdf(
          ...
          children: [
            pw.Expanded(
              child: pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('$label (${data.labels.farmStandard})',
                      style: pw.TextStyle(
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          // Cabeçalho
          pw.Text(data.labels.reportTitle,
              style:
          ...
          // Título da sessão
          pw.Text(data.labels.sectionResults,
              style:
  ...
  static pw.Widget _chip(String title, String value) {
    return pw.Container(
      decoration: pw.BoxDecoration(
        ...
        borderRadius: pw.BorderRadius.circular(8),
        border: pw.Border.all(color: PdfColors.white.withOpacity(0.25)),
      ),
  ...
  static String _money(Object? v) {
    ...
    // formatação simples: $ x,xx
    return r'$ ' + n.toStringAsFixed(2).replaceAll('.', ',');
    // Se quiser locale-aware: passe o texto já formatado no map
  }
